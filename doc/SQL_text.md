以下は、実際の利用例や各関数・テーブルの定義内容（Supabase のスニペット一覧や使用例を参照）を反映させた修正版の **SQL_text.md** の例です。各セクションではテーブル定義、テーブル修正、関数定義（集計・解析・補助用関数）およびトリガーの作成、さらに使用例をまとめています。

```markdown:doc/SQL_text.md
# SQL 定義と使用例
